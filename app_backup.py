...
if opcion == "🏠 Dashboard":
    ...
    st.info("Desde aquí puedes crear, renombrar o eliminar acuarios. Antes de eliminar se intentará crear un backup en la carpeta `backups/`.")
    ...
    if not acuarios:
        ...
        with col2:
            st.warning("Eliminar un acuario eliminará sus ficheros locales. Se creará un backup automáticamente si es posible.")
            ...
            if st.button("Eliminar acuario seleccionado", key="admin_delete_btn"):
                if not confirmar:
                    ...
                    try:
                        backup_path = eliminar_acuario(seleccionado)
                        if backup_path:
                            st.success(f"Acuario '{seleccionado}' eliminado. Backup creado: {backup_path}")
                        else:
                            st.success(f"Acuario '{seleccionado}' eliminado. No se creó backup o no se encontró nada para respaldar.")
                    except Exception as e:
